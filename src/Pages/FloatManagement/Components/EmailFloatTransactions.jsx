...
const ACTION_STYLE = {
  monthly_credit: "bg-green-100 text-green-800 border-green-200",
...
const ActionBadge = ({ action }) => {
  ...
  const label = action === "monthly_credit" ? "monthly credit" : action;
  return (
...
const EmailFloatTransactions = ({ emailAccountId, onAccountResolved }) => {
  const axiosPrivate = useAxiosPrivate();
  const [page, setPage]   = useState(1);
  const [limit, setLimit] = useState(10);

  const { data, isLoading, isError, error, isFetching } = useQuery({
    queryKey: ["email-float-transactions", emailAccountId, page, limit],
    queryFn: async () => {
      const res = await axiosPrivate.get("/float-management/email-float/transactions", {
        params: { email_account_id: emailAccountId, page, limit },
      });
  ...
  useEffect(() => {
    if (data?.email_account?.name && onAccountResolved) {
      onAccountResolved(data.email_account);
    }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        <CardTitle>
          {data?.email_account?.name
            ? `${data.email_account.name} Transactions`
            : "Email Transactions"}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Charge/Email</TableHead>
              ...
              <TableBody>
                {rows.map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell className="text-right font-medium">{tx.email_units ?? 0}</TableCell>
                    <TableCell className="text-right">{currency(tx.charge_per_email)}</TableCell>
...
export default EmailFloatTransactions;
