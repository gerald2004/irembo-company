...
const WithdrawTransactionDialog = ({ isOpen, onClose, refetch, accountId, handleOpenReceiptDialog }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axiosPrivate.post("/accounting/withdraw/withdraws", payload);
  ...
  const stepIcons = [
    { icon: <Info className="w-6 h-6 text-blue-500" />,      label: "Withdraw Details" },
