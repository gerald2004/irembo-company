...
const DepositTransactionDialog = ({ isOpen, onClose, refetch, accountId, handleOpenReceiptDialog }) => {
  ...
  const [chargesCount, setChargesCount] = useState(null); // null = not yet loaded

  const watchedAmount = parseFloat(watch("deposit_transaction_amount")) || 0;

  const handleChargesLoaded = useCallback((count) => setChargesCount(count), []);

  const hasCharges = chargesCount === null || chargesCount > 0;
  ...
  const goBack = () => {
    if (step === 3 && !hasCharges) setStep(1);
    else setStep((p) => Math.max(p - 1, 1));
  };

  const handleClose = () => {
    reset(); setStep(1); setSkipFeeIds([]); setFeeOverrides({});
    setSelectedChannel(null); setChannelError(""); setChargesCount(null);
    onClose();
  };
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center space-x-3 my-1">
          {stepLabels.map((label, i) => (
            <div key={label} className={`flex items-center transition-opacity ${visualStep >= i + 1 ? "opacity-100" : "opacity-50"}`}>
              {stepIcons[i]}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <Input id="deposit_transaction_notes" placeholder="Optional note"
