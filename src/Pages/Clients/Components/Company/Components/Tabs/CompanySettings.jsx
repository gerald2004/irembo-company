...
const DeleteCompanyDialog = ({ companyName, onDelete }) => {
  const [open, setOpen] = useState(false);
  const [inputName, setInputName] = useState("");
  const [confirm, setConfirm] = useState("");

  const handleDelete = () => {
    if (inputName !== companyName || confirm !== "delete this account") {
        ...
        <div className="space-y-4 mt-4">
          <div>
            <label className="text-sm">
              Enter company name <span className="font-bold">{companyName}</span> to continue:
            </label>
            <Input
...
const CompanySettings = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (updates) => {
      setLoading(true);
      const res = await axiosPrivate.patch(`/clients/company/${id}`, updates);
      return res.data.data.client;
    },
    onSuccess: () => {
      toast({ title: "Success", description: "Settings updated successfully." });
      queryClient.invalidateQueries(["company-settings", id]);
      queryClient.invalidateQueries(["company-client", id]);
    },
    onError: () => {
  ...
  const handleDelete = async () => {
    try {
      const res = await axiosPrivate.delete(`/clients/company/${id}`);
      ...
      <Card>
        ...
        <CardFooter className="flex justify-between">
          ...
          {hasPermission(roles, 100010) && (
            <DeleteCompanyDialog
              companyName={settings?.client_firstname ?? ""}
