...
import { useState, useRef } from "react";import { formatDateTimestamp } from "@/lib/utils";
...
const WritternOffLoansReport = () => {
 ...
 const tableRef = useRef(null);
  ...
  const {
    ...
    queryKey: ["writtern-off-loans", filters],
    queryFn: async () => {
      const controller = new AbortController();
      const fetchURL = `reports/loans/writternoff-loans`;
      try {
  ...
  const totalAmountCleared = data?.reduce(
    (sum, loan) => sum + loan.amount_cleared,
    0
  );
  ...
  const columns = [
    ...
    {
      accessorKey: "disbursed_on",
      header: "Date Of Disbursement",
      cell: ({ row }) => (
        <p className="capitalize text-xs">
          {formatDateTimestamp(row.original.disbursed_on)}
        </p>
      ),
    },
    {
    ...
    {
      accessorKey: "amount_cleared",
      header: "Amount Cleared",
      cell: ({ row }) => (
        <p className="capitalize text-xs">
          {parseFloat(row.original.amount_cleared).toLocaleString()}
        </p>
      ),
    },
    {
      accessorKey: "writtenoff_by",
      header: "Writtenoff By",
      cell: ({ row }) => (
        <p className="capitalize text-xs">{row.original.writtenoff_by}</p>
      ),
    },
    {
      accessorKey: "reason",
      header: "Reason",
      cell: ({ row }) => (
        <p className="capitalize text-xs">{row.original.reason}</p>
      ),
    },
    {
      accessorKey: "notes",
      header: "Notes",
      cell: ({ row }) => (
        <p className="capitalize text-xs">{row.original.notes}</p>
      ),
    },

    {
      accessorKey: "timestamp",
      header: "Writtern Off On",
      cell: ({ row }) => (
        <p className="capitalize text-xs">
    ...
    <>
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Writtern Off Loans Reports</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
        ...
        <div className="flex-1 space-y-4 p-0 pt-2">
          ...
          <LoanGeneralReportQuery
            ...
            totals={{
              ...
              totalAmountCleared: totalAmountCleared,
            }}
            title={"Writtern Off Loans Report"}
          />
