...
const Datatable = forwardRef(({
  ...
  const { totalDebit, totalCredit } = useMemo(() => {
    if (!data || data.length === 0) return { totalDebit: 0, totalCredit: 0 };
    ...
    return data.reduce(
      (totals, row) => {
        ...
        } else if (row.debit_credit === "Credit") {
          ...
          <TableBody>
            {isLoading || isRefetching ? (
              ...
              <>
                ...
                {/* ✅ Summary Row (Only if data has debits/credits) */}
                {data.some((row) => row.debit_credit) && (
                  <TableRow className="font-semibold">
                    <TableCell colSpan={columns.length - colSpan}></TableCell>
