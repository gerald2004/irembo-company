...
const LoanDocuments = ({ isOpen, isClose }) => {
  ...
  const { data: documents = [], isLoading, isError, refetch, isRefetching } = useQuery({
    ...
    queryFn: async ({ signal }) => {
      const response = await axiosPrivate.get(`/loans/${loanId}/documents`, { signal });
      return response.data.data.documents || [];
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (docId) => axiosPrivate.delete(`/loans/documents/${docId}`),
    onMutate: async (docId) => {
      await queryClient.cancelQueries({ queryKey: ["loanDocuments", loanId] });
      const prev = queryClient.getQueryData(["loanDocuments", loanId]);
      queryClient.setQueryData(["loanDocuments", loanId], (old) =>
        old?.filter((d) => d.document_id !== docId)
      );
      return { prev };
    },
    onError: (_, __, context) => {
      queryClient.setQueryData(["loanDocuments", loanId], context.prev);
      toast({ title: "Deletion failed", variant: "destructive" });
    },
    onSettled: () => queryClient.invalidateQueries({ queryKey: ["loanDocuments", loanId] }),
  });
  ...
  const canDelete  = hasPermission(roles, 100091);
