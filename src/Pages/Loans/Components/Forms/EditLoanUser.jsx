  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          <DialogTitle>Update Loan Officer</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {/* Step 1: Loan user Details */}
          {step === 1 && (
            <fieldset className="grid grid-cols-1 md:grid-cols-1 gap-4">
              <div>
                <Label htmlFor="user_id">Select Loan Officer</Label>
                <Controller
                  name="user_id"
                  control={control}
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Select a loan officer">
                          {users?.find(
                            (item) =>
                              String(item.user_id) === String(field.value)
                          )?.user_firstname || "Select User"}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        {isLoading ? (
