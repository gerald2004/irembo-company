...
const AddLoanDocuments = ({ isOpen, isClose, refetch }) => {
  ...
  const addDocumentMutation = useMutation({
    mutationFn: async (formData) => {
      ...
      const response = await axiosPrivate.post(`/loans/documents`, formData, {
        headers: {
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(["loanDocuments", loanId]); // Refresh documents list
      reset();
    ...
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new document for this loan.
          </DialogDescription>
          <DialogClose asChild>
            <button
