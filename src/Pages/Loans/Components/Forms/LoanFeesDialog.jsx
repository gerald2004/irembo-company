...
const LoanFeesDialog = ({ isOpen, onClose, refetch }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axiosPrivate.post("/loans/charges", payload, {
  ...
  const [selectedAccounts, setSelectedAccounts] = useState({
    loan_income_account: null,
    loan_receivable_account: null,
  });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Loan Fees </DialogTitle>
          <DialogDescription>
            Follow the steps to add loan fees.
          </DialogDescription>
          <DialogClose asChild>
            <button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {step === 1 && (
            <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="loan_charge_amount">Amount</Label>
                <Input
                  id="loan_charge_amount"
                  type="number"
                  placeholder="Enter amount"
                  {...register("loan_charge_amount", {
                    required: "Amount is required",
                  })}
                />
                {errors.loan_charge_amount && (
                  <p className="text-red-500 text-sm">
                    {errors.loan_charge_amount.message}
                  </p>
                )}
              </div>
              <div>
                <Label htmlFor="loan_charge_title">Reason</Label>
                <Input
                  id="loan_charge_title"
                  type="text"
                  placeholder="Enter reason"
                  {...register("loan_charge_title", {
                    required: "reason is required",
                  })}
                />
                {errors.loan_charge_title && (
                  <p className="text-red-500 text-sm">
                    {errors.loan_charge_title.message}
                  </p>
                )}
              </div>
              <AccountCombobox
                label="Income Account"
                selectedAccount={selectedAccounts.loan_income_account}
                onAccountSelect={(value) =>
                  setSelectedAccounts((prev) => ({
                    ...prev,
                    loan_income_account: parseInt(value),
                  }))
                }
              ...
              <AccountCombobox
                ...
                selectedAccount={selectedAccounts.loan_receivable_account}
                onAccountSelect={(value) =>
                  setSelectedAccounts((prev) => ({
                    ...prev,
                    loan_receivable_account: value,
                  }))
                }
                accountsData={accountsData}
                isLoading={isLoadingAccounts}
