...
const LoanDisbursementDialog = ({
  ...
  const {
    ...
    defaultValues: {
      date: new Date().toISOString().split("T")[0], // Set default date as today
    },
  });
    ...
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Follow the steps to complete the disburse loan application.
          </DialogDescription>
          <DialogClose asChild>
            <button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {step === 1 && (
            <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* interest percentage */}
              <div>
                <Label htmlFor="loan_products_interest_rate">
                  Interest (%)
                </Label>
                <Input
                  id="loan_products_interest_rate"
                  type="number"
                  placeholder="Enter interest in percentage"
                  {...register("loan_products_interest_rate", {
                    required: "Interest percentage is required",
                  })}
                />
                {errors.loan_products_interest_rate && (
                  <p className="text-red-500 text-sm">
                    {errors.loan_products_interest_rate.message}
                  </p>
                )}
              </div>
              {/* date disbutrsement */}
