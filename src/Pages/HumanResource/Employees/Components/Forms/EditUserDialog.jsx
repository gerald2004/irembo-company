...
const EditUserDialog = ({ isOpen, onClose, refetch, defaultValues }) => {
  ...
  const onSubmitImage = async (formData) => {
    ...
    try {
      await axiosPrivate.post(
        `/business/employees/${defaultValues?.user_id}`,
        formData,
        {
          headers: {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      data.department = data.user_department;
      ...
      if (data.user_image instanceof File) {
        formData.append("user_image", data.user_image); // Append file
        isImageChanged = true;
      }
  ...
  useEffect(() => {
    if (defaultValues) {
      // ✅ Field Mapping: API Keys → Form Keys
      const fieldMapping = {
        ...
        image: "user_image",
        can_login: "user_can_login",
      };

      // ✅ Set Basic Form Fields
      Object.entries(fieldMapping).forEach(([formKey, apiKey]) => {
        let value = defaultValues;
        apiKey.split(".").forEach((key) => {
          value = value?.[key]; // Handle nested objects
        });
      ...
      // ✅ Ensure `branches` are loaded before setting `branch_id`
      if (branches.length > 0 && defaultValues.branch?.branch_id) {
        const existingBranch = branches.find(
          (branch) =>
            String(branch.id) === String(defaultValues.branch.branch_id)
        );

        if (existingBranch) {
          setValue("branch_id", String(defaultValues.branch.branch_id));
          setSelectedBranch(String(defaultValues.branch.branch_id)); // ✅ Fix: Ensure selectedBranch updates
        }
      }

      // ✅ Ensure `roles` are loaded before setting `role_id`
      if (roles.length > 0 && defaultValues.role?.role_id) {
        const existingRole = roles.find(
          (role) => String(role.id) === String(defaultValues.role.role_id)
        );

        if (existingRole) {
          setValue("role_id", String(defaultValues.role.role_id));
        }
      }

      // ✅ Ensure `departments` are loaded before setting `user_department`
      if (departments.length > 0 && defaultValues.department?.department_id) {
        const existingDepartment = departments.find(
          (department) =>
            String(department.id) ===
            String(defaultValues.department.department_id)
        );
  ...
  }, [defaultValues, branches, roles, departments, setValue]);
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill in the required fields to edit staff.
          </DialogDescription>
          <DialogClose asChild>
            <button
            ...
            <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Controller
                  ...
                  render={({ field }) => {
                    // ✅ Handle invalid dates (0000-00-00 or empty values)
                    let parsedDate = null;
                    if (field.value && field.value !== "0000-00-00") {
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant={"outline"} className="w-full">
                            {parsedDate
                              ? parsedDate.toLocaleDateString() // Format the date properly
                              : "Pick a date"}
              ...
              <div>
                <Label>Gender</Label>
                <Select
                  defaultValue={defaultValues?.user_gender}
          ...
          {step === 2 && (
            <fieldset className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>Status</Label>
                <Select
                  defaultValue={defaultValues?.user_status}
            ...
            <fieldset className="grid grid-cols-1 md:grid-cols-1 gap-4">
              <div className="flex flex-col items-center space-y-2">
                ...
                <div className="relative w-32 h-32 border border-gray-300 rounded-full overflow-hidden flex items-center justify-center">
                  ...
                  ) : defaultValues?.user_image ? (
                    <img
                      src={defaultValues.user_image}
                      alt="User Image"
                ...
                <input
                  ...
                  className="hidden"
                  id="file-upload"
                  onChange={handleFileChange} // ✅ Ensure correct file handling
                />
